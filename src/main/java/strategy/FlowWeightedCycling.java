...
public class FlowWeightedCycling implements strategy.Strategy {
	...
	public FlowWeightedCycling(double phase_length, StreetMap street_map) {
	...
	private void setTrafficLightActivity(Intersection inter, HashMap<Road, ArrayList<Car>> list_of_cars) {
		...
		int total_flow = 0;
		for (double v : inter.getPreviousFlows().values()) total_flow += v;
		double prev_flow = inter.getPreviousFlows().get(road_for_new_active);
		double weight = prev_flow / Math.max(1, total_flow);
