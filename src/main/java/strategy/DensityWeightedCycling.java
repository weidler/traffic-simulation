...
public class DensityWeightedCycling implements Strategy {
	...
	public DensityWeightedCycling(double phase_length, StreetMap street_map) {
	...
	private void setTrafficLightActivity(Intersection inter, HashMap<Road, ArrayList<Car>> list_of_cars) {
		...
		for (Road r : inter.getOutgoingRoads()) {
			for (Car c : list_of_cars.get(r)) {
				if (c.getCurrentDestinationIntersection() == inter) {
					total_queued++;
				}
			}
		}
