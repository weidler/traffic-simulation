...
public class IntelligentDriverModel {
	...
	public double getAcceleration(Car car, double dist_leading, double leading_velocity) {
		...
		double free_term = car.getMaxAcceleration() * (1 - Math.pow((car.getCurrentVelocity() / car.getDesiredVelocity()), delta));
		...
		double desired_distance =  (min_spacing + car.getCurrentVelocity()) * min_headway + (car.getCurrentVelocity() * (car.getCurrentVelocity() - leading_velocity))/(2 * Math.sqrt(car.getMaxAcceleration() * car.getDecceleration()));
