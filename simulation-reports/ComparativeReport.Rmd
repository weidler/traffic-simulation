...
```{r include=FALSE}
srcFiles <- c("Basic.csv", "Informed.csv", "Weighted.csv", "FlowWeighted.csv", "QueueWeightedCycling.csv", "DensityWeightedCycling.csv")
...
for (i_exp in 1:length(data_frames)) {
  ...
  avg_frac_waits[i_exp] <- mean(frac_waits[[i_exp]]$mean)
  avg_avg_speeds[i_exp] <- mean(avg_speeds[[i_exp]]$mean)
  avg_numb_cars[i_exp] <- mean(numb_cars[[i_exp]]$mean)
  ...
  var_frac_waits[i_exp] <- mean(frac_waits[[i_exp]]$var)
  var_avg_speeds[i_exp] <- mean(avg_speeds[[i_exp]]$var)
  var_numb_cars[i_exp] <- mean(numb_cars[[i_exp]]$var)
}
```
