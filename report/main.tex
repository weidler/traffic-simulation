...
\textbf{Dynamic+}   & 11.29*$^\dagger$          & 134.06            & 0.325                     
\end{tabular}
\vspace{20pt}
\centering
...
\textbf{Informed}   & 0.1229 & 0.0166            & 0.0004                     \\
...
\textbf{Dynamic+}   & \textbf{0.0930*$^\dagger$}          & 0.012             & 0.0002                    
\end{tabular}
\end{table}

On the first of three maps, the informed cycling strategy outperforms all other strategies significantly in average velocity. Notably, Dynamic+ is a very close runner-up with a difference of only 0.29. Except for Dynamic, all strategies outperform the Basic Cycling benchmark system.

With respect to the fraction of cars waiting, Dynamic+ is the best numerically best performing strategy, outperforming both benchmark strategies significantly. All other strategies can outperform the basic cycling, but informed is significantly the second best strategy for that measure.
...
The Tie-Fighter map yields similar results to those of Brusselsepoort. Best-prforming system measured in average speed is flow-weighted cyclingm followed by informed cycling. Except for the dynamic strategies, all strategies significantly outperform basic cycling. The average fraction of cars waiting on this map has its best score for queue weighted cycling, but flow weighted cycling follows iwth a difference of only 0.0008. For both measures, all systems except for the dynamic strategies outperform basic cycling significantly.
